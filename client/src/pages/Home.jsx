...
import { User, ExternalLink, RefreshCw, Compass } from 'lucide-react';
        ...
        <AnimatePresence>
          {metrics && !loading && (
            ...
            >
              ...
              <div className="glass-card p-6 border border-slate-200/80 dark:border-slate-800 rounded-3xl flex flex-col sm:flex-row items-center justify-between gap-4 hover-lift glow-card">
                <div className="flex items-center gap-4 text-left">
                  <img
                    src={metrics.avatar}
                    alt={metrics.profileName}
                    className="w-14 h-14 rounded-2xl object-cover ring-2 ring-indigo-500/20 border border-slate-200 dark:border-slate-800 shrink-0"
                  />
                  <div>
                    ...
                    {metrics.profileUrl && (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="text-[11px] font-bold text-indigo-500 hover:underline inline-flex items-center gap-1 mt-1"
                      >
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto">
                  <button
                    ...
                    className="flex-1 sm:flex-none py-2.5 px-4 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 font-bold text-xs hover:bg-slate-100 dark:hover:bg-slate-900 transition-colors flex items-center justify-center gap-1.5"
                  >
                  ...
                  <button
                    onClick={handleReset}
                    className="flex-1 sm:flex-none py-2.5 px-4 rounded-xl bg-slate-100 dark:bg-slate-900 text-slate-600 dark:text-slate-400 font-bold text-xs hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors"
                  >
                    Reset
                  </button>
                </div>
              ...
              {/* 2. Points Breakdown & Tier Progress Grid */}
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <PointsBreakdown metrics={metrics} />
                <TierProgress metrics={metrics} />
              </div>

              {/* 3. Badge Tracker & Facilitator Progress Grid */}
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
